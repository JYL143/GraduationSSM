...
<div id="page-wrapper">
    <div class="container" id="addform">
        ...
        <div>
            <span style="color: black;font-size: 25px">分类(无商品)管理</span>
        </div>
        <form  action="#" method="post">

                <div style="float: left">
                    <span style="padding-top: 10px ;margin-left:400px;display: inline-block;color: #0F9E5E">请输入要添加的分类名称:</span>
                </div>
                <div class="search-box" style="width: 90px;margin-left: 20px">
                    <input class=" input__field--madoka"  style="width: 90px;" name="catname" type="text" id="input-34" placeholder="分类名称" />
                ...
                <div>
                    ...
                    <div style="float: left">
                        <input  style="width: 70px;height: 35px;margin-top: 7px;margin-left: 40px;" type="submit"  id="add2" class="btn btn-success"value="添加"  />
                    </div>
                    ...
                    <c:forEach items="${pageinfo.list}" var="cat">
                        <tr>
                            ...
                            <th>
                                ...
                                <button onclick="delete_cat(${cat['cat_id']},'${cat['cat_name']}')"   class="btn btn-danger btn-sm"  >删除</button>
                            </th>
                        </tr>
                    </c:forEach>
                    </thead>
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination">
                        ...
                        <c:if test="${!pageinfo.hasPreviousPage}"> <%-- 是否有上一页，没有就显示禁用状态--%>
                            <li class="disabled"><a   href="/cat/findcs2">首页</a></li>
                            <li class="disabled" >
                                <a  href="/cat/findcs2?pn=${pageinfo.pageNum-1}" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                        </c:if>
                        <c:if test="${pageinfo.hasPreviousPage}"> <%-- 是否有上一页，有就显示正常--%>
                            <li><a  href="/cat/findcs2">首页</a></li>
                            <li>
                                <a href="/cat/findcs2?pn=${pageinfo.pageNum-1}" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                        ...
                        <c:forEach items="${pageinfo.navigatepageNums}" var="page_Num">
                            ...
                            <c:if test="${page_Num!=pageinfo.pageNum}">
                                <li><a href="/cat/findcs2?pn=${page_Num}">${page_Num}</a></li>
                            </c:if>
                        ...
                        <c:if test="${!pageinfo.hasNextPage}">
                            <li class="disabled">
                                <a href="/cat/findcs2?pn=${pageinfo.pageNum+1}" aria-label="Next">
                                    <span aria-hidden="true">&raquo;</span>
                                </a>
                            </li>
                            <li class="disabled"><a href="/cat/findcs2?pn=${pageinfo.pages}">末页</a></li>
                        </c:if>
                        ...
                        <c:if test="${pageinfo.hasNextPage}">
                            ...
                            <li><a href="/cat/findcs2?pn=${pageinfo.pages}">末页</a></li>
                        </c:if>
...
<script type="text/javascript">
    var k; //全局变量保存点击修改时的id
    var k2; //全局变量保存点击删除时的id
    var pageNUM=document.getElementById('pageNUM').innerHTML;
    var total=document.getElementById('total').innerHTML;
    //-----------------------------------------------删除方法-----------------------------------------------------------------
    function delete_cat(catid,catname) {

        k2=catid;
        var catname = catname;   //获取被点击删除的分类名

        // 弹出是否确认删除对话框
        if(confirm("确认删除【"+catname+"】吗？")){
            //确认，发送ajax请求删除即可
            $.ajax({
                url:"/cat/deletecat?catid="+k2,
                type:"POST",
                success:function(){
                    alert("删除成功!");
                    if ((total-1)%9==0) {
                    ...
                    window.location.href="/cat/findcs2?pn="+pageNUM;//跳转到当前页码那一页
                }
            });
        }
    }
    ...
    $("#add2").click(function () {
        $.ajax({
            ...
            success:function(result){//99表示正常，100表示重复
                if (result==100){
                    alert("该分类名字已存在，清换个分类名字!");
                    window.location.href="/cat/findcs2?pn="+pageNUM;//跳转到当前页码那一页
                }else{
                    alert("添加成功!");
                    window.location.href="/cat/findcs2?pn="+result;//跳转到当前页码那一页
                }
